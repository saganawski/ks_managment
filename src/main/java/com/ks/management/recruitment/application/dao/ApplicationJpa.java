...
public interface ApplicationJpa extends JpaRepository<Application,Integer> {
    ...
    @Query(value = "Select ap.id, ap.first_name, ap.last_name, ap.phone_number, ap.email, ap.date_received, ap.call_back_date, ap.created_date, s.source, ar.result, o.name FROM application AS ap " +
            "JOIN application_source AS s ON s.id = ap.application_source_id " +
            "JOIN application_result AS ar ON ar.id = ap.application_result_id " +
